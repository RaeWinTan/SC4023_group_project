...
class DataBase:
    ...
    def write_data(self, x,y,row_number, columns, aggregation_column_names,aggregation_columns, aggregation_functions, data_types,
                   output_names):
        values = [] 
        for name in columns:
            cs:ColumnObject = self.column_stores[name]
    ...
    def query(self, x, y, mat_id):#query either give back -1 or a row_number, -1 means no result
        ...
        rtn = zone_map_search(zones, 
                              ...
                              [change_fn(year_month_change_fn, ["inRange"]), change_fn(price_per_area_change_fn,["isLesserThanEqualToUpperBound","isLesserThanUpperBound"]), 
                               change_fn(identity_change_fn,["inRange"]), change_fn(identity_change_fn, ["inRange"])],
                              l,r)
        return rtn 
