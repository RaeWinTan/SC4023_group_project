...
def zone_map_search(
    ...
    conditions,
    values: List[ComparatorFunction],
    condition_call_backs,
    change_fns,
    l,
    r,
):
    ...
    while idx < r + 1:
        if not all(
            [zm.checkInside(idx, conditions[i]) for i, zm in enumerate(zone_maps)]
        ):
            """
            ...
            for j in range(idx, end, VECTOR_SIZE):
                tmp = vectorization(
                    j,
                    min(j + VECTOR_SIZE - 1, end - 1),
                    values,
                    conditions,
                    current_call_back_indexes,
                    condition_call_backs,
                    change_fns,
                )
                if tmp != -1:
...
def vectorization(
    ...
    conditions_values,
    current_call_back_indexes,
    conditions_call_backs,
    change_fn,
):  # l,r is somewhat define by the zone map
    rtn = -1
    for idx in range(l, r + 1):
        if all(
            [
                c.comparator_callbacks[
                    conditions_call_backs[i][current_call_back_indexes[i]]
                ](idx, conditions_values[i])
            ...
            for i in range(len(values)):
                conditions_values[i], current_call_back_indexes[i] = change_fn[i](
                    values[i].get(idx), conditions_values[i]
                )
                rtn = idx
    return rtn
